...
if (isset($_POST['update'])) {
  ...
  if ($jumlah >= $qtyskrg) {
    ...
    if ($kurangistock >= 0) {
      ...
      $updatedata1 = mysqli_query($conn, "update sbrg_keluar set jumlah='$jumlah', penerima='$tujuan',keterangan='$keterangan' where id='$id'");
      ...
      if ($updatedata1 && $queryx) {
        echo "<div class='alert alert-success'>
              ...
              <meta http-equiv='refresh' content='5; url= keluar.php'/>  ";
      } else {
    ...
    $updatedata = mysqli_query($conn, "update sbrg_keluar set jumlah='$jumlah', penerima='$tujuan', keterangan='$keterangan' where id='$id'");
...
<head>
  ...
  <title>Barang Keluar | Cipta Selaras Funiture</title>
  ...
  <div class="page-container">
        ...
        <div class="menu-inner">
          <nav>
            <ul class="metismenu" id="menu">
              ...
              <?php
              // Jika role-nya super admin, maka tampilkan menu Tambah User
    ...
    <div class="main-content">
      ...
      if (isset($_GET['status'])) {
        if ($_GET['status'] == "qty-lebih-besar-dari-stock") {
          // echo "<div class='alert alert-danger alert-dismissible fade show'><button type='button' class='close' data-dismiss='alert'>&times;</button>Stok  <strong><u>" . $data['nama'] . "</u> <u>" . ($data['merk']) . "</u> &nbsp <u>" . $data['ukuran'] . "</u></strong> yang tersisa sudah habis</div>";

          echo "<div class='alert alert-danger alert-dismissible fade show'><button type='button' class='close' data-dismiss='alert'>&times;</button>Jumlah keluar lebih besar dari stock barang</div>";
        }
      }
      ?>
            ...
            <div class="card">
              <div class="card-body">
                <div class="d-sm-flex justify-content-between align-items-center">
                  <h2>Barang Keluar</h2>
                  <button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="btn btn-info col-md-2"><span class="ti-plus"></span> Tambah</button>
                </div>
                <div class="data-tables datatable-dark">
                  <div class="table-responsive">
                    <table id="dataTable3" class="table table-hover">
                      ...
                      <tbody>
                        <?php
                        $brg = mysqli_query($conn, "SELECT sbrg_keluar.id, sbrg_keluar.idx, sbrg_keluar.tgl, sbrg_keluar.jumlah, sbrg_keluar.penerima, sbrg_keluar.keterangan, sstock_brg.idx, sstock_brg.nama, sstock_brg.jenis, sstock_brg.merk, sstock_brg.ukuran FROM sbrg_keluar JOIN sstock_brg ON sbrg_keluar.idx = sstock_brg.idx ORDER BY sbrg_keluar.id DESC");
                        $no = 1;
                        ...
                        ?>
                          ...
                          <tr>
                            ...
                            <td><button data-toggle="modal" data-target="#edit<?= $id; ?>" class="btn btn-primary ti-pencil"></button> <button data-toggle="modal" data-target="#del<?= $id; ?>" class="btn btn-danger ti-trash"></button></td>
                          </tr>
                              ...
                              <div class="modal-content">
                                <form method="post">
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <label class="form-control"><?php echo $b['tgl'] ?></label>

                                    <label for="nama">Barang</label>
                                    <label class="form-control"><?php echo $b['nama'] ?> <?php echo $b['merk'] ?> <?php echo $b['jenis'] ?></label>

                                    <label for="ukuran">Ukuran</label>
                                    <label class="form-control"><?php echo $b['ukuran'] ?></label>

                                    <label for="jumlah">Jumlah</label>
                                    <input type="number" id="jumlah" name="jumlah" class="form-control" value="<?php echo $b['jumlah'] ?>">

                                    <label for="tujuan">Tujuan</label>
                                    <input type="text" name="tujuan" class="form-control" value="<?php echo $b['penerima'] ?>">
                ...
                <a href="exportbrgklr.php" target="_blank" class="btn btn-info mt-4">Export Data</a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="modal-body">
          <form action="barang_keluar_act.php" method="post">
            <div class="form-group">
              <label>Tanggal</label>
              <!-- <input name="tanggal" type="text" class="form-control" value="<?php echo date('d-m-Y') ?>" disabled> -->
              <input name="tanggal" type="date" class="form-control" required>
            </div>
            <div class="form-group">
              <label>Nama Barang</label>
              <select name="barang" class="custom-select form-control" style="height: 3.3em;">
                <option selected>Pilih barang</option>
                <?php
                $det = mysqli_query($conn, "select * from sstock_brg order by nama ASC") or die(mysqli_error($conn));
                while ($d = mysqli_fetch_array($det)) {
                  echo '<option value="' . $d['idx'] . '">' . $d['nama'] . '' . $d['merk'] . '' . $d['jenis'] . ', Uk.' . $d['ukuran'] . '  --- Stock : ' . $d['stock'] . '</option>';
                }
                ?>
              </select>
            </div>
            ...
            <div class="form-group">
              <label>Tujuan</label>
              <!-- <input name="penerima" type="text" class="form-control" placeholder="Tujuan barang"> -->
              <select name="tujuan" class="custom-select form-control" style="height: 3.3em;" required>
                <option></option>
